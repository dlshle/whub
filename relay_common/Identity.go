...
type IWRBaseRole interface {
	...
	Description() string
	Type() int
	DraftMessage(from string, to string, uri string, msgType int, payload []byte) *messages.Message
}
...
func (c *WRBaseRole) DraftMessage(from string, to string, uri string, msgType int, payload []byte) *messages.Message {
	return messages.NewMessage(utils.GenStringId(), from, to, uri, msgType, payload)
}
