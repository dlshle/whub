...
func (m *ConnectionManagerModule) OnLoad() {
	if err := middleware_manager.RegisterMiddleware(new(ConnectionMiddleware)); err != nil {
		m.Logger().Printf("unable to register connection middleware due to %s", err.Error())
	}
	m.ModuleBase.OnLoad()
}
