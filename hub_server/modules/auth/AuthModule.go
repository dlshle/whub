...
func (c *AuthModule) OnLoad() {
	if err := middleware_manager.RegisterMiddleware(new(AuthMiddleware)); err != nil {
		c.Logger().Printf("unable to register auth middleware due to %s", err.Error())
	}
	c.ModuleBase.OnLoad()
}
