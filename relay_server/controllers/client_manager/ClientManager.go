...
func (m *ClientManager) AcceptClient(id string, client *client.Client) error {
	...
	m.logger.Printf("client_manager (%s, %s) has been accepted %v", id, client.Address(), client.Describe())
	return nil
}
...
func (m *ClientManager) DisconnectClient(id string) (err error) {
	...
	defer m.logger.Printf("error while disconnecting client_manager %s due to %v", id, err)
	if client == nil {
...
func (m *ClientManager) HandleClientConnectionClosed(c *client.Client, err error) {
	if err == nil {
		...
		// unexpected closure
		// service_manager should kill all jobs and transit to DeadMode
		m.withWrite(func() {
